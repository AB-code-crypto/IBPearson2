...
def _start_of_ct_week(now_ct: datetime) -> datetime:
    day_start = now_ct.replace(hour=0, minute=0, second=0, microsecond=0)
    return day_start - timedelta(days=now_ct.weekday())


def _start_of_ct_month(now_ct: datetime) -> datetime:
    return now_ct.replace(day=1, hour=0, minute=0, second=0, microsecond=0)


def _is_last_friday_of_month(now_ct: datetime) -> bool:
    return now_ct.weekday() == 4 and (now_ct + timedelta(days=7)).month != now_ct.month
...
def _is_weekly_due(now_ct: datetime) -> bool:
    return now_ct.weekday() == 4 and now_ct.hour == 16 and now_ct.minute == 1


def _is_monthly_due(now_ct: datetime) -> bool:
    return _is_last_friday_of_month(now_ct) and now_ct.hour == 16 and now_ct.minute == 2
            ...
            try:
                if _is_daily_due(now_msk):
                    ...
                    if daily_key != last_daily_key:
                        ...
                        summary = _fetch_trade_summary(
                            ...
                            start_ts=_utc_ts(start_utc),
                            end_ts=_utc_ts(now_utc),
                        )
                ...
                if _is_weekly_due(now_ct):
                    ...
                    if weekly_key != last_weekly_key:
                        start_ct = _start_of_ct_week(now_ct)
                        summary = _fetch_trade_summary(
                            db_path=settings.trade_db_path,
                            instrument_code=instrument_code,
                            start_ts=_utc_ts(start_ct),
                            end_ts=_utc_ts(now_utc),
                        )
                        text = _build_summary_text(
                            ...
                            instrument_code=instrument_code,
                            period_label=(
                                f"CT {_format_dt(start_ct)} → {_format_dt(now_ct)}"
                            ),
                            summary=summary,
                        )
                        await _send_summary_to_targets(sender=sender, settings=settings, text=text)
                ...
                if _is_monthly_due(now_ct):
                    monthly_key = f"{now_ct.year}-{now_ct.month:02d}"
                    if monthly_key != last_monthly_key:
                        start_ct = _start_of_ct_month(now_ct)
                        summary = _fetch_trade_summary(
                            db_path=settings.trade_db_path,
                            instrument_code=instrument_code,
                            start_ts=_utc_ts(start_ct),
