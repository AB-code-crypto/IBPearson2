...
class TelegramSender:
    ...
    async def _post_photo(self, photo_path, chat_id, caption=""):
    ...
    async def send_photo(self, photo_path, chat_id=None):
        # Отправка только картинки без текста.
        resolved_chat_id = self._resolve_chat_id(chat_id)
        return await self._post_photo(photo_path, resolved_chat_id)

    async def send_photo_with_text(self, photo_path, text, chat_id=None):
        ...
        if not text:
            return await self.send_photo(photo_path, chat_id=resolved_chat_id)

        # Если текст помещается в caption — шлём фото с подписью.
        if len(text) <= TELEGRAM_CAPTION_LIMIT:
            return await self._post_photo(photo_path, resolved_chat_id, caption=text)

        # Если текст длинный:
        # 1) отправляем картинку без подписи
        # 2) отдельно отправляем текст с автоматической нарезкой
        photo_ok = await self._post_photo(photo_path, resolved_chat_id)
        text_ok = await self.send_text(text, chat_id=resolved_chat_id)
