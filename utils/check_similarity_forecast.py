...
INSTRUMENT_CODE = "MNQ"
CURRENT_HOUR_START_TEXT_UTC = "2026-03-17 14:00:00"
TARGET_BAR_INDEX = 359

# Сколько кандидатов после similarity берем в прогнозный слой.
FORECAST_TOP_N = 10

# Если нужно, можно отсекать слабых по Пирсону до similarity-рейтинга.
...
def build_plot_file_path(current_hour_start_text_utc, target_bar_index):
    ...
    file_name = f"forecast_similarity_{safe_hour_text}_bar_{target_bar_index}.png"
...
def main():
    ...
    try:
        ...
        selected_candidates = ranked_similarity_candidates[:FORECAST_TOP_N]

        selected_prepared_hours = []

        for item in selected_candidates:
            selected_prepared_hours.append(prepared_hours_map[item["hour_start_ts"]])
