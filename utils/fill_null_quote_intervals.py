'''
скрипт для ремонта истории, где есть строки с отсутствующими ASK и/или BID ценами. Он находит проблемные строки, группирует их в подряд идущие интервалы
отдельно по сторонам ASK и BID, затем докачивает эти интервалы из IB кусками и записывает обратно только недостающую сторону котировок.
'''
import asyncio
import sqlite3
